...
define([
  ...
  describe('sulfur/schema/type/double', function () {
    ...
    it("should be derived from sulfur/schema/type/_simple", function () {
      expect($_simpleType).to.be.prototypeOf($doubleType);
    });

    describe('.getLegalFacets()', function () {

      it("should include sulfur/schema/facet/enumeration", function () {
        expect($doubleType.getLegalFacets()).to.include($enumerationFacet);
      });

      it("should include sulfur/schema/facet/maxExclusive", function () {
        expect($doubleType.getLegalFacets()).to.include($maxExclusiveFacet);
      });

      it("should include sulfur/schema/facet/maxInclusive", function () {
        expect($doubleType.getLegalFacets()).to.include($maxInclusiveFacet);
      });

      it("should include sulfur/schema/facet/minExclusive", function () {
        expect($doubleType.getLegalFacets()).to.include($minExclusiveFacet);
      });

      it("should include sulfur/schema/facet/minInclusive", function () {
        expect($doubleType.getLegalFacets()).to.include($minInclusiveFacet);
      });

      it("should include sulfur/schema/facet/pattern", function () {
        expect($doubleType.getLegalFacets()).to.include($patternFacet);
      });
    ...
    describe('.getValueType()', function () {
      ...
      it("should be sulfur/schema/value/double", function () {
        expect($doubleType.getValueType()).to.equal($doubleValue);
      });
